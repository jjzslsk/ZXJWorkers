 ...
 * isMinDateCur 是否限制不能选择小于当前时间
 */
function initDatePicker(viewId,type,funCal,frmName,isMinDateCur){
	if(isMinDateCur==undefined || isMinDateCur==null || isMinDateCur=='' || typeof isMinDateCur!= "boolean"){
		isMinDateCur=false;
	}
	var calendar = new datePicker();
	var cdDate = new Date();
	...
	if(isMinDateCur){
		var monthCur=cdDate.getMonth() + 1;//月份 
		var dateCur=cdDate.getDate();//日
		minDate=yealCur+'-'+monthCur+'-'+dateCur;
	}
